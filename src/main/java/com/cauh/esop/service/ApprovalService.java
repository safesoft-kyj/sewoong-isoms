...
import com.cauh.esop.repository.*;
            ...
            case SOP_RD_Request_Form:
                ...
                if(sopRdRequestForm.isNewSOPDevelopment()) {
                    for(SopRdDevelopmentDoc rd : sopRdRequestForm.getRdDevelopmentDocs()) {
                        ...
                        rd.setDocumentType(DocumentType.RD);
...
//            documentVersionService.retirement(rdDocument);
    ...
    private void sopRdRequestForm(Approval approval) {
        SopRdRequestForm sopRdRequestForm = approval.getSopRdRequestForm();
        ...
        //신규 개발 RD 정보 등록
        if(sopRdRequestForm.isNewRDDevelopment()) {
            for(SopRdDevelopmentDoc rd : sopRdRequestForm.getRdDevelopmentDocs()) {
                Document document = new Document();
                ...
                document.setType(DocumentType.RD);

                Document savedRd = documentRepository.save(document);
                DocumentVersion rdVer = new DocumentVersion();
                rdVer.setId(UUID.randomUUID().toString());
                rdVer.setDocument(savedRd);
                rdVer.setStatus(DocumentStatus.DEVELOPMENT);
                rdVer.setVersion(rd.getVersion());
                rdVer.setTrainingAll(false);
                documentVersionRepository.save(rdVer);
                log.debug("@신규 개발 RD 정보 등록");
            }
        }

        if(sopRdRequestForm.isSopRevision()) {
            List<SopRdRevisionDoc> sopRevisionDocs = sopRdRequestForm.getSopRevisionDocs();
...
//                documentVersion.setVersion(Double.toString(Double.parseDouble(supersededVersion.getVersion()) + 0.1));
        ...
        if(sopRdRequestForm.isRdRevision()) {
            List<SopRdRevisionDoc> rdRevisionDocs = sopRdRequestForm.getRdRevisionDocs();
