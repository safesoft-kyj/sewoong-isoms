...
import com.cauh.esop.domain.constant.NoticeStatus;
...
import com.cauh.esop.security.annotation.IsAllowedRD;
import com.cauh.esop.security.annotation.IsAllowedSOP;
...
public class DocumentVersionService {
    ...
    public DocumentVersion save(DocumentVersion documentVersion) {
        ...
        } else if (documentVersion.getDocument().getType() == DocumentType.RD) {
...
//    }
    ...
    public Iterable<DocumentVersion> findAllCurrentRDList(List<String> rdIds) {
        BooleanBuilder builder = new BooleanBuilder();
        QDocumentVersion qDocumentVersion = QDocumentVersion.documentVersion;
        builder.and(qDocumentVersion.document.type.eq(DocumentType.RD));
...
//        }
    ...
    public BooleanBuilder getMainRDPredicate(DocumentStatus status, List<String> sopIdList) {
        BooleanBuilder builder = new BooleanBuilder();
...
//        }
    ...
    public void approvedToEffective() {
        ...
        List<DocumentVersion> effectiveRDs = getApprovedToEffectiveDocuments(DocumentType.RD, now);
...
//            sb.append(" 및 RD");
        ...
        Iterable<RetirementDocument> retirementRDDocuments = retirementDocumentService.findRetirementDocs(now, DocumentType.RD);

        boolean hasRetirementSOPs = ObjectUtils.isEmpty(retirementSOPDocuments) == false;
        boolean hasRetirementRDs = ObjectUtils.isEmpty(retirementRDDocuments) == false;
        if(hasRetirementSOPs) {
            retirementDocumentService.retired(retirementSOPDocuments);
        }
        if(hasRetirementRDs) {
            retirementDocumentService.retired(retirementRDDocuments);
        }
...
//            sb.append(" 공지의 건");
            ...
            model.put("retirementRDs", retirementRDDocuments);
...
//            mailService.sendMail(mail);
            ...
            if(!ObjectUtils.isEmpty(effectiveRDs)) {
                Iterable<DocumentVersion> iterable = findAll(getPredicate(DocumentType.RD, DocumentStatus.EFFECTIVE, null, null, null));
                List<DocumentVersion> documentVersions = StreamSupport.stream(iterable.spliterator(), false).collect(Collectors.toList());
                model.clear();
