...
@Subselect("select a.user_id, max(b.id) training_period_id from(\n" +
        ...
        "\t\tselect distinct jd.user_id, m.document_version_id from(\n" +
        "\t\t\tselect a.user_id, b.id as jd_id \n" +
        "\t\t\tfrom c_user_job_description a inner join c_job_description b\n" +
        "\t\t\ton a.job_description_id = b.id\n" +
        "\t\t\twhere a.status = 'APPROVED'\n" +
        "\t\t) jd inner join s_training_matrix m\n" +
